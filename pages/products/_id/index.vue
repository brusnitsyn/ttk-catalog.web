    ...
    <div class="flex flex-col lg:flex-row gap-x-24 pb-3">
      ...
      <div class="flex flex-col flex-grow px-2 py-1">
        ...
        <div v-if="product.properties.length > 0">
          <h2 class="pb-1.5 pt-3 font-lg font-inter font-semibold">
            Характеристики
          </h2>
        ...
        <div v-if="product.machines.length > 0" class="pt-2">
          <h2 class="pb-1.5 pt-3 font-lg font-inter font-semibold">
            Применяемость
          </h2>
          <div class="flex flex-col md:flex-row gap-x-1">
            <span
              v-for="machine in product.machines"
              :key="machine.id"
              class="font-inter"
            >
              {{ machine.name }}
  ...
  watch: {
    product: function (value) {
      if(value.images.length > 0)
        ...
        this.selectImage('/img/no-finded-image.png')
    ...
    handleChange(value) {
      ...
      if(value !== null && typeof value !== 'undefined') {
        if (this.product.discountPrice <= 0)
          ...
          this.price = this.product.discountPrice * value
      }
    },
  },
  async fetch() {
