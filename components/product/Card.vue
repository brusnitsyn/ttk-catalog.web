  ...
  <div class="my-px mx-px">
    <div class="flex rounded-[4px] h-full shadow-[0_0_2px_0px_rgba(0,0,0,.10)]" :class="classTemplate">
      <nuxt-link :to="{ name: 'catalog-product-id', params: { id: product.id } }" class="cursor-pointer w-full">
        ...
        rounded-[4px]
        select-none
        relative" :class="{ 'pt-10': product.category.id != 1 }">
          <div class="flex absolute left-1.5 top-1.5 z-10" v-if="product.category && product.category.id != 1">
            <div :style="{
          ...
          <el-image v-if="product.images.length > 0" :src="product.images[0].url" alt="" class="w-24" :z-index="0"
            :fit="'cover'">
          </el-image>
          <el-image v-else src="/img/no-finded-image.png" alt="" class="w-24" :z-index="0" :fit="'cover'">
          </el-image>
        </div>
      </nuxt-link>

      <div class="flex flex-col px-2 py-2 gap-y-2 h-full justify-between">
        <div class="flex flex-col text-sm gap-y-1">
          <span class="break-words max-h-9 font-medium leading-[18px] text-ellipsis overflow-hidden">
            {{ product.name }}
          </span>
          <span class="break-words max-h-9 leading-[18px] text-ellipsis overflow-hidden">
            {{ product.article }}
          </span>
        ...
        <div class="flex flex-row justify-between max-h-9">
          <el-button type="primary" size="small" @click="addProductToBasket(product)">В корзину</el-button>
  ...
  methods: {
    addProductToBasket(product) {
      ...
      if (product.discountPrice > 0)
        this.$store.dispatch('basket/addProduct', { product, qty, price: product.discountPrice })
      else
        this.$store.dispatch('basket/addProduct', { product, qty, price: product.actualPrice })
    },
  }
}
</script>
